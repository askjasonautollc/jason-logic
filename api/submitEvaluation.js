...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      // ✅ Handle photo uploads and attach as 'attachments'
      if (files.photos) {
        let uploads = Array.isArray(files.photos) ? files.photos : [files.photos];
        uploads = uploads.slice(0, 2).filter(
          file => file && file.size > 0 && file.mimetype?.startsWith("image/")
        );
        ...
        for (const photo of uploads) {
          ...
          await openai.beta.threads.messages.create(thread.id, {
            role: "user",
            content: "Vehicle photo for evaluation.",
            attachments: [
              {
                file_id: uploadedFile.id,
                tools: [{ type: "code_interpreter" }] // or tool needed by your assistant
              }
            ]
          });
        }
